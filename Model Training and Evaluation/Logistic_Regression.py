...
def train_model(train_data, model_output_base_dir, dataset_name, feature_type, seed):
    ...
    if feature_type == 'tfidf':
        ...
        # 使用 nlp.pipe 进行批处理，这比单条处理更稳定且快
        print("Computing embeddings using nlp.pipe...")
        docs = list(nlp.pipe(train_data['text'], batch_size=100))
...
def evaluate_model(test_data, model_output_dir, result_output_base_dir, dataset_name, feature_type, seed):
    ...
    if feature_type == 'tfidf':
        ...
        # 使用 nlp.pipe 进行批处理，解决内存崩溃问题
        print("Computing evaluation embeddings using nlp.pipe...")
        docs = list(nlp.pipe(test_data['text'], batch_size=100))
        X_test = np.array([doc.vector for doc in docs])
    ...
    # 如果目录不存在，则创建它（包括所有中间父目录）
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    # ========================

    df_report.to_csv(result_file_path)  # 这是原来的报错行
    print(f"Classification report saved to {result_file_path}")
